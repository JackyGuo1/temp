...
object SimpleApp {
  ...
  def main(args: Array[String]) {
    ...
    def iterFunc(x: Seq[String]) = {
      var list = new LinkedList[(Long, String)]
      for (i <- x) { list.add((nonNegativeMod(i.##, 1 << 20), i)) }
      Collections.sort(list, new Comparator[(Long, String)]() {
        def compare(x: (Long, String), y: (Long, String)) = {
          (x._1 - y._1).toInt
        }
      })
      list.asScala
    ...
    documents.coalesce(1, false)
    println(documents.partitions)
    documents.saveAsTextFile("./wordSegmentFile")

    val ids: RDD[Long] = documents.map((tuple: (Int, Seq[String])) => tuple._1)
    ...
    val id_hashid = ids.zip(descriptions.map(iterFunc))
